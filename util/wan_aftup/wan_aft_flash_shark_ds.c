...
aftup_flash_t	aft_shark_ds_flash = {  0x014F, AFT_CORE_X1000_SIZE };
...
aft_erase_flash_shark(wan_aft_cpld_t *cpld, int stype, int verify)
{
	...
	for(sector_no = 0; sector_no < 19; sector_no++){
		if (aft_shark_flash_ds_spec[cpld->flash_index][sector_no].sector_type != stype){
			continue;
		}
